...
const requiredFields = ['ID', 'Year', 'Month', 'Day', 'Eggs', 'Rejected', 'Sold', 'Others', 'Flocks', 'Cages'];
...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const result = (await axios.get('http://localhost:5001/general/dashboard')).data;
      setRows(result);
      setLoading(false);
  ...
  const uploadData = async () => {
    try {
      ...
      const jokesResponse = (await axios.get('http://localhost:5001/api/jokes')).data;
      ...
      if (updatedJokes.length) {
        promises.push(
          axios.post('http://localhost:5001/bulk/jokes-bulk-update', updatedJokes)
        );
      }
      if (newJokes.length) {
        promises.push(
  ...
  return (
    <Box m="1.5rem 2.5rem">
      <FlexBetween>
        <Header title="DASHBOARD" subtitle="Poultry Monitoring System Dashboard" />
      ...
      <Dialog open={selectedFile && !confirmedImport} onClose={openDialog}>
        ...
        <DialogContent>
          ...
          Are you sure you want to import data from <strong>{fileName}</strong>?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
        <Button onClick={openDialog} style={{ fontSize: '18px' }} color="error">
          Cancel
      </Button>
      <Button onClick={handleConfirmImport} style={{ fontSize: '18px' }} color="success">
        Confirm
      </Button>
        </DialogActions>
      </Dialog>
      ...
      >
        ...
        >
          ...
          >
            Breakdown of real states and information via category for eggs
